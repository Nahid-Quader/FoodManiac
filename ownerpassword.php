...
$sql="SELECT password FROM owner  WHERE owner.ownerkey=('$ownerkey')";
...
if($preValue==$row['password'] )
{
	...
	{
		$sql="UPDATE `foodmaniac`.`owner` SET `password` = '".$newValue2."'
		WHERE `owner`.`ownerkey` = '".$ownerkey."' AND `owner`.`password`='".$row['password']."'";
